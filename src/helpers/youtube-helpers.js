...
export async function getYoutubeSearchResults(query) {
  ...
  const maxResults = queryParams.has("maxResults")
    ? parseInt(queryParams.get("maxResults"))
    : 5;
  ...
  const youtubeSearchURL = new URL(
  ...
  let response = await fetch(youtubeSearchURL.href, fetchOptions).then(
    (res) => {
      if (!res.ok) {
